...
class GAN :
	...
	encoder_input = T.concatenate([cropped_image, mask], axis=1) # shoudl concat wrt channels
        ...
        if version == 1:  
            if encode : 
                ...
                gen_layers.append(nn.batch_norm(ll.Conv2DLayer(gen_layers[-1], 4000, 4, 4, pad=1, nonlinearity=nn.lrelu)))  # 1024 x 2 x 2 --> 2048 x 1 x 1       
                ...
                gen_layers.append(nn.batch_norm(nn.Deconv2DLayer(gen_layers[-1], (self.batch_size, 128*4, 4, 4),(5,5), stride=(4,4)))) 
		# concat with noise
                latent_size = 2048 
	    ...
	    mixing_coefs = [theano.shared(lasagne.utils.floatX(0.05)) for i in range(2)]
	    
	    # theano.shared(lasagne.utils.floatX(np.array([0.5])))  for i in range(3)]
	    mixing_coefs.append(theano.shared(lasagne.utils.floatX(1)))
            border = 2
            gen_layers.append(nn.batch_norm(nn.Deconv2DLayer(gen_layers[-1], (self.batch_size, 128*2, 8, 8), (5, 5), W=Normal(0.02),nonlinearity=nn.relu), g=None))  # 4 -> 8
	    ...
	    #gen_layers.append(layer_concat_a)        
            ...
            gen_layers.append(nn.batch_norm(nn.Deconv2DLayer(gen_layers[-1], (self.batch_size, 128, 16, 16), (5, 5), W=Normal(0.02),nonlinearity=nn.relu), g=None))  # 8 -> 16
	    ...
	    #gen_layers.append(layer_concat_b)        
            ...
            gen_layers.append(nn.batch_norm(nn.Deconv2DLayer(gen_layers[-1], (self.batch_size, 64, 32, 32), (5, 5), W=Normal(0.02),nonlinearity=nn.relu), g=None))  # 16 -> 32            
    ...
    def print_mixing_coefs(self):
	critic = self.critic
	coefs = []
	for param in ll.get_all_params(self.generator[-1]):
	    if "mix" in param.name:
		coefs.append(param.get_value().mean())
