...
num_epochs = 50
version = ''
name = 'DCGAN' + str(version)
load_params = False
last_saved_epoch = 2300
initial_eta = 1e-4
...
if load_params:
    ...
    path_gen =  'gen_eor1.npz'#iname + '_gen_' + last_saved_epoch_str + '.npz'#
...
hid_fake = ll.get_output(GAN.critic[4], inputs=critic_input, deterministic=False)
...
gen_loss = lasagne.objectives.binary_crossentropy(fake_out, 1).mean()  + 10 * loss_gen_fm
#gen_loss = lasagne.objectives.binary_crossentropy(fake_out_small, 1).mean()  + 10 * loss_gen_fm
...
for epoch in range(0, 30000) :
    ...
    for i in range(1):
        ...
        samples = samples*255#67.75 + 113.91# 255.#6
        #iraw_samples = raw_samples *67.75 + 113.91# 255.#
